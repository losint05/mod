...
if (!ses) {message.channel.send(new MessageEmbed().setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription("**<@"+strg.id+"> Bir Sesli Kanalda Değil!** <a:milona_carpi:825673698299215882>"));}
if (ses) {message.channel.send(new MessageEmbed().setColor('#7289D').setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription("**<@"+strg.id+"> İsimli Kişi `"+ses.name+"` İsimli Kanalda!** <a:milona_yesiltik2:827479367852359730>"));}};
